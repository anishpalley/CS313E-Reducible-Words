...
def insert_word(s, hash_table):
    ...
    while hash_table[hash_index] != "":
        hash_index = (hash_index + step_size(s))%len(hash_table)
    ...
    #     hash_table[hash_index] = s
    # else:
    #     new_index = (hash_index + step)%len(hash_table)
    #     slots_checked = 1
    #     while hash_table[new_index] is not None and hash_table[new_index] != s:
    #         new_index = (new_index + step)%len(hash_table)
    #         if hash_table[new_index] is None:
    #             hash_table[new_index] = s
    #             return
    #         if slots_checked >= len(hash_table):
    #             return
    #         slots_checked += 1
...
def is_reducible(s, hash_table, hash_memo):
    ...
    def helper(word, hash_table, hash_memo):
        if word == "a" or word == "o" or word == "i" or find_word(word, hash_memo):
            return True
        if find_word(word, hash_table) is False or len(word) == 1:
            return False
        for i in range(len(word)):
            new_word = word[:i] + word[i+1:]
            if helper(new_word, hash_table, hash_memo):
